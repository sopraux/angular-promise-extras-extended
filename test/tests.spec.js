  ...
  describe('$q.allSettled', function() {
    ...
    it('works when passed an array', function(done) {
      ...
      var testProm = $q.allSettled(proms).then(function(results) {
        ...
        expect(results[0].isRejected).toBe(false)
        expect(results[1].isFulfilled).toBe(false)
        expect(results[1].isRejected).toBe(true)
        expect(results[2].isFulfilled).toBe(true)
        expect(results[2].isRejected).toBe(false)
      })
      .then(done, done.fail)
    ...
    it('works when passed a hash', function(done) {
      ...
      var testProm = $q.allSettled(promsObj).then(function(results) {
        ...
        expect(results.a.isFulfilled).toBe(true)
        expect(results.a.isRejected).toBe(false)
        expect(results.b.isFulfilled).toBe(false)
        expect(results.b.isRejected).toBe(true)
        expect(results.c.isFulfilled).toBe(true)
        expect(results.c.isRejected).toBe(false)
